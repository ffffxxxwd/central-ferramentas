/* ============================================================
   Leads do WhatsApp — organizador de contatos/clientes
   ------------------------------------------------------------
   - Tudo é salvo localmente no navegador (localStorage).
   - Sem dependências externas: funciona offline.
   - Etiquetas coloridas (você escolhe a cor), como no WhatsApp.
   - Exportar/Importar backup pelo botão "Backup".
   ============================================================ */
(function () {
  "use strict";

  /* ---------- Configuração ---------- */
  var STORAGE_KEY = "leads_whatsapp_v1";
  ...
  // Paleta das etiquetas — cores escolhidas pelo usuário (visíveis no claro e no escuro)
  var PALETA = ["#e5484d","#f76b15","#c99700","#17994f","#0d9488","#2f6fed","#7c5cff","#d6409f","#6b7280"];

  /* ---------- Estado ---------- */
  var state = {
    leads: [],
    view: "lista",     // lista (Contatos) | agenda
    filtro: "todos",   // todos | <id de status>
    busca: ""
  };
  var editId = null;        // id em edição (null = novo)
  var drawerDraft = null;   // cópia de trabalho enquanto o painel está aberto
  var drawerCorSel = PALETA[0];
  var toastTimer = null;
  ...
  function numeroWhats(tel){
    var d = soDigitos(tel);
    if (!d) return "";
    if (d.length === 10 || d.length === 11) d = "55" + d; // acrescenta DDI do Brasil p/ número local
    return d;
  }
  ...
  // Adicionar rápido: cola o número (ou "Nome 11 99999-8888") e cria o contato na hora.
  // Aceita vários por vez, um por linha.
  function quickAdd(){
    if (!el.quick) return;
    ...
    raw.split(/[\n;]+/).forEach(function (linha){
      ...
      if (tel){
        ...
        var existe = state.leads.some(function (l){ return l.telefone && numeroWhats(l.telefone) === norm; });
        if (existe){ dup++; return; }
      }
  ...
  function ordenar(arr){
    return arr.slice().sort(function (a, b){
      var da = a.proximaAcao || "9999-99-99";
      var db = b.proximaAcao || "9999-99-99";
  ...
  function grupoAgenda(titulo, arr, cls){
    ...
    return '<div class="ag-grupo ag-' + cls + '">' +
      ...
      '<div class="lista">' + arr.map(leadRow).join("") + "</div>" +
    "</div>";
  }
  ...
  function leadRow(l){
    ...
    var tags = (l.etiquetas && l.etiquetas.length)
      ? '<div class="lead-tags">' + l.etiquetas.map(tagInline).join("") + "</div>" : "";
    ...
    var acoes = "";
    if (wa) acoes += '<a class="btn btn-sm" href="' + wa + '" target="_blank" rel="noopener">WhatsApp</a>';
    ...
    return '<div class="lead">' +
      '<div class="lead-main" data-acao="detalhe" data-id="' + l.id + '">' +
        '<div class="lead-l1">' +
          '<span class="lead-nome">' + escapeHtml(l.nome || "(sem nome)") + "</span>" +
          '<span class="lead-status' + statusCls + '">' + escapeHtml(st.nome) + "</span>" +
        "</div>" +
        tags +
        ...
        obs +
      "</div>" +
      '<div class="lead-actions">' + acoes + "</div>" +
    "</div>";
  }
  ...
  function vazio(msg){
    ...
    return '<div class="empty">' +
      ...
      "<p>Cadastre os clientes que chegam pelo WhatsApp: nome, número, o que combinaram e quando falar de novo. " +
      "Fica tudo visível aqui — sem precisar abrir cada conversa pra lembrar.</p>" +
  ...
  function drawerHtml(l){
    ...
    var hist = (isEdit && l.historico && l.historico.length)
      ? l.historico.map(function (h){
          return '<div class="hist-item"><span class="hist-data">' + fmtDataHora(h.data) + "</span>" + escapeHtml(h.texto) + "</div>";
        }).join("")
      : "";

    return '<div class="drawer-head">' +
        "<h2>" + (isEdit ? "Editar contato" : "Novo contato") + "</h2>" +
        '<button class="icon-btn" data-acao="fechar-drawer" aria-label="Fechar">✕</button>' +
      "</div>" +
      '<div class="drawer-body">' +
        ...
        "</div>" +
        campo("Observações", "f_obs", '<textarea id="f_obs" placeholder="Anotações que ficam à mostra na lista: nº de contrato, o que o cliente pediu, horário bom pra ligar...">' + escapeHtml(l.observacoes) + "</textarea>") +
        etiquetasHtml(l) +
        (isEdit ?
          '<div class="hist-bloco">' +
            '<div class="hist-head">Histórico</div>' +
            '<div class="hist-add">' +
              '<input id="f_histnovo" type="text" placeholder="Registrar uma anotação rápida...">' +
              '<button class="btn btn-sm" data-acao="add-hist">Registrar</button>' +
            "</div>" +
      ...
      '<div class="drawer-foot">' +
        ...
        '<div class="foot-dir">' +
          '<button class="btn" data-acao="fechar-drawer">Cancelar</button>' +
          '<button class="btn btn-primary" data-acao="salvar">Salvar</button>' +
        "</div>" +
      "</div>";
  }
  ...
  function etiquetasHtml(l){
    ...
    return '<div class="etq-bloco">' +
      '<span class="campo-lab">Etiquetas <span class="campo-hint">(cor que você escolhe, aparece na lista)</span></span>' +
      atuais +
      '<div class="etq-add">' +
        '<input id="f_etqtexto" type="text" placeholder="Nome da etiqueta (opcional)">' +
        '<div class="swatches">' + swatches + "</div>" +
        '<button class="btn btn-sm" data-acao="etq-add">Adicionar</button>' +
      "</div>" +
      sugestoesHtml(l) +
    "</div>";
  }
  ...
  function salvarDrawer(){
    ...
    if (editId){
      ...
      var novo = {
        id: uid(), nome: drawerDraft.nome, telefone: drawerDraft.telefone, email: drawerDraft.email, empresa: drawerDraft.empresa,
  ...
  function carregarExemplos(){
    ...
    var ex = [
      { nome:"Maria Souza",  telefone:"11912345678", email:"maria@email.com",    empresa:"Indicação", status:"conversando", proximaAcao:maisDias(0),  proximaAcaoNota:"Ligar pra confirmar interesse", observacoes:"Prefere ligação depois das 18h. Já enviou os documentos.", etiquetas:[{texto:"Quente", cor:"#e5484d"}] },
      { nome:"João Pereira", telefone:"21998877665", email:"",                   empresa:"GAV",       status:"proposta",    proximaAcao:maisDias(-2), proximaAcaoNota:"Enviar contrato revisado",      observacoes:"Contrato 12345. Pediu desconto na entrada.",            etiquetas:[{texto:"Pagamento pendente", cor:"#c99700"}] },
      { nome:"Ana Lima",     telefone:"31991112222", email:"ana.lima@email.com", empresa:"WAM",       status:"novo",        proximaAcao:maisDias(2),  proximaAcaoNota:"Mandar apresentação",           observacoes:"Chegou pelo anúncio do Instagram.",                     etiquetas:[{texto:"", cor:"#2f6fed"}] }
    ];
    ...
    salvar(); render(); toast("Exemplos adicionados. Explore e depois é só apagar.");
  }
  ...
  function aplicarTema(){
    var t = "claro";
    try { t = localStorage.getItem("tema") || "claro"; } catch (e){}
  ...
  function init(){
    ...
    window.addEventListener("storage", function (e){ if (e.key === "tema") aplicarTema(); });

    // Recebe contatos capturados pela extensão do WhatsApp Web (se estiver instalada).
    // A extensão injeta uma "ponte" nesta página que envia a fila via postMessage.
    window.addEventListener("message", function (e){
      var d = e.data;
      if (!d || d.source !== "leads-ext" || d.tipo !== "capturas" || !d.itens) return;
      var ids = [], criados = 0;
      d.itens.forEach(function (it){
        ...
        var l = { id:uid(), nome:nome, telefone:tel, email:"", empresa:"", status:"novo",
                  proximaAcao:"", proximaAcaoNota:"", observacoes:"", etiquetas:[], historico:[], criadoEm:agora(), atualizadoEm:agora() };
        addHist(l, "Recebido do WhatsApp (extensão)");
        state.leads.push(l);
        criados++;
      });
      if (criados){ salvar(); render(); toast(criados === 1 ? "1 contato novo do WhatsApp." : criados + " contatos novos do WhatsApp."); }
      // devolve os ids recebidos pra extensão limpar a fila (mesmo os que já existiam)
      if (ids.length) window.postMessage({ source:"leads-tool", tipo:"ack", ids: ids }, "*");
    });
  }
